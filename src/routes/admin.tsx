...
import { checkAdminPassword, analyzeProductImage } from "@/lib/adminAnalyze.functions";
import { CATEGORIES as SITE_CATEGORIES } from "@/data/products";
  ...
  return (
    <div className="rounded-xl border border-border bg-background shadow-sm">
      <div className="flex items-center gap-3 border-b border-border p-4">
        ...
        <div>
          <div className="text-sm font-medium text-foreground">Ảnh Danh Mục</div>
          <div className="text-xs text-muted-foreground">Upload ảnh riêng cho từng danh mục hiển thị trên trang chủ</div>
        </div>
      </div>
      <div className="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {SITE_CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat.id} className="rounded-lg border border-border bg-cream/30 p-3">
              <div className="relative mb-2 aspect-[3/4] overflow-hidden rounded-lg bg-cream">
                {currentImg ? (
                  <>
                    <img src={currentImg} alt={cat.label} className="h-full w-full object-cover" />
                    <button
                      type="button"
                      onClick={() => removeCatImage(cat.id)}
              ...
              <div className="text-center">
                ...
                <label className="mt-1.5 inline-flex cursor-pointer items-center gap-1 rounded-full border border-border bg-background px-3 py-1 text-[11px] text-muted-foreground transition hover:border-primary hover:text-primary">
      ...
      <div className="border-t border-border px-4 py-2">
        <p className="text-[11px] text-muted-foreground">
          Ảnh sẽ được lưu trong trình duyệt (localStorage). Nên dùng ảnh tỉ lệ 3:4, kích thước ~400×533px.
        </p>
      </div>
    </div>
  );
