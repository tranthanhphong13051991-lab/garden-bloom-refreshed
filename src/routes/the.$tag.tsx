...
import { findTag, TAGS, type Tag } from "@/data/tags";
...
export const Route = createFileRoute("/the/$tag")({
  ...
  head: ({ loaderData }) => {
    ...
    const itemListLd = {
      ...
      itemListElement: tag.products.map((p: Product, i: number) => ({
        "@type": "ListItem",
        position: i + 1,
...
function TagPage() {
  const { tag } = Route.useLoaderData() as { tag: Tag };
  const popular = TAGS.filter((t: Tag) => t.slug !== tag.slug).slice(0, 12);
      ...
      <section className="py-12">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {tag.products.map((p: Product) => <ProductCard key={p.slug} product={p} />)}
          </div>
        </div>
      </section>
