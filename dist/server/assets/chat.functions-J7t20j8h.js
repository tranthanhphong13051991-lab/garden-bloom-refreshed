...
import { z } from "zod";
import { P as PRODUCTS, a as formatPrice } from "./products-c_hw6lyT.js";
...
const SYSTEM_PROMPT = `Mày tên Ngọc, nhân viên tư vấn tại Hoa Tươi Thanh Ngọc (8 Phan Văn Hân, Bình Thạnh, TP.HCM).
...
const SUGGEST_TOOL = {
  ...
  function: {
    ...
    description: "Gợi ý 1-3 sản phẩm hoa phù hợp với nhu cầu của khách. Chỉ dùng slug có trong danh sách sản phẩm đã cho.",
    parameters: {
      type: "object",
      properties: {
        slugs: {
...
async function getNgocResponse(userMessage, history = []) {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
    return {
      reply: "Ngọc đang gặp chút vấn đề kết nối AI. Bạn nhắn Zalo 0934 926 092 để được tư vấn ngay nhé 🌸",
      products: []
    };
  }
  ...
  if (!res.ok) {
    return {
      reply: "Ngọc đang bận chút xíu, bạn nhắn Zalo 0934 926 092 để được tư vấn ngay nhé 🌸",
      products: []
    };
  }
  ...
  const validSlugs = slugs.filter((s) => typeof s === "string" && ALLOWED_SLUGS.includes(s)).filter((s) => seen.has(s) ? false : (seen.add(s), true)).slice(0, 3);
  const products = validSlugs.map((slug) => PRODUCTS.find((p) => p.slug === slug)).map((p) => ({
    slug: p.slug,
    name: p.name,
    thumb: p.thumb,
...
const chatWithFlorist_createServerFn_handler = createServerRpc({
  id: "b8c6ee5cd835032fe39f1196ebb398701c7cffa33d6f448016e6c7e0f95deec3",
  name: "chatWithFlorist",
  filename: "src/lib/chat.functions.ts"
}, (opts) => chatWithFlorist.__executeServer(opts));
const chatWithFlorist = createServerFn({
  method: "POST"
}).inputValidator((data) => InputSchema.parse(data)).handler(chatWithFlorist_createServerFn_handler, async ({
  data
}) => {
...
export {
  chatWithFlorist_createServerFn_handler
};
